...
| Request struct naming | `<Service><Method>Request`, using the **renamed** method (e.g. `OrdersListRequest`, `AccountsGetRequest`). Service-qualified for package-level uniqueness (all types share one package; a bare `ListRequest` would collide across services). Suffix unified on `Request` (drops the old `Params`). Methods with no input have no request struct. |
| Response types | **Not renamed** — response bodies are generated by oapi-codegen (`DO NOT EDIT`); keep their domain names and return `*T`. The `XxxResponse` in the signature is illustrative, not a rename mandate. |
...
- Request struct naming: `<Method>Request` (no service qualifier — flat, names
  already unique). Suffix unified on `Request` (drops `Params`): e.g.
  `GetAssetParams` → `GetAssetRequest`, `ListAssetsParams` → `ListAssetsRequest`.
...
- Rename request structs to `<Service><Method>Request` with the new method names
  (`AccountsGetAccountRequest` → `AccountsGetRequest`, `OrdersPlaceOrderRequest`
  → `OrdersPlaceRequest`, …).
...
- Requests already value — no change to pass-by-value; **rename the param
  structs** to `<Service><Method>Request` (`GetOrdersParams` → `OrdersListRequest`,
  `PlaceMarketOrderParams` → `OrdersPlaceMarketRequest`, `GetOrderBookParams` →
